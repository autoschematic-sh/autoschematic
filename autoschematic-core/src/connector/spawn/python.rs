...
impl ResourceAddress for PythonResourceAddress {
    ...
    where
        Self: Sized,
    {
        Ok(Some(PythonResourceAddress(path.to_path_buf())))
    }
}
...
impl Resource for PythonResource {
    ...
    fn from_str(_addr: &impl ResourceAddress, s: &str) -> Result<Self, anyhow::Error>
    where
        Self: Sized,
    {
...
impl Connector for PythonConnector {
    ...
    async fn op_exec(&self, addr: &Path, op: &str) -> Result<OpExecOutput, anyhow::Error> {
        let res = Python::with_gil(|py| -> anyhow::Result<OpExecOutput> {
            ...
            let res = self
                .object
                .call_method1(py, pyo3::intern!(py, "op_exec"), args);
            ...
            match res {
                ...
                Err(e) => {
                    if let Some(traceback) = e.traceback(py) {
                        ...
                        Err(anyhow!(
                            "Python connector exception: op_exec(): {}",
                            e.to_string()
                        ))
                    }
                }
    ...
    async fn get(&self, addr: &Path) -> Result<Option<GetResourceOutput>, anyhow::Error> {
        let res = Python::with_gil(|py| -> Result<Option<GetResourceOutput>, anyhow::Error> {
            ...
            match res {
                Ok(r) => {
                    if r.is_truthy(py)? {
                        let outputs: Option<HashMap<String, Option<String>>> =
                            match r.getattr(py, "outputs") {
                                Ok(val) => val.extract(py)?,
                                Err(_) => None,
                            };
                        let resource_definition: String =
                            r.getattr(py, "resource_definition")?.extract(py)?;
                ...
                Err(e) => {
                    if let Some(traceback) = e.traceback(py) {
                        ...
                        Err(anyhow!(
                            "Python connector exception: get(): {}",
                            e.to_string()
                        ))
                    }
                }
    ...
    async fn list(&self, subpath: &Path) -> Result<Vec<PathBuf>, anyhow::Error> {
        let res = Python::with_gil(|py| -> anyhow::Result<Vec<PathBuf>> {
            ...
            let res = self
                .object
                .call_method1(py, pyo3::intern!(py, "list"), args);
            ...
            match res {
                ...
                Err(e) => {
                    if let Some(traceback) = e.traceback(py) {
                        ...
                        Err(anyhow!(
                            "Python connector exception: list(): {}",
                            e.to_string()
                        ))
                    }
                }
    ...
    async fn eq(&self, addr: &Path, a: &str, b: &str) -> anyhow::Result<bool> {
        let res = Python::with_gil(|py| -> anyhow::Result<bool> {
            let logger = LoggingStdout::default().into_pyobject(py)?;
            let sys = py.import("sys")?;
            sys.setattr("stdout", logger)?;
            ...
            let args = PyTuple::new(
                py,
                [addr.to_string_lossy().into(), a.to_string(), b.to_string()],
            )?;
    ...
    async fn diag(&self, addr: &Path, a: &str) -> anyhow::Result<DiagnosticOutput> {
        let res = Python::with_gil(|py| -> anyhow::Result<DiagnosticOutput> {
            ...
            let args = PyTuple::new(
                py,
                [addr.to_string_lossy().into(), a.to_string()],
            )?;
