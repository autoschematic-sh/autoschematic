...
use crate::{
    task::message::from_github_webhook,
    template::{
        ApplyOverallSuccessTemplate, FilterMatchedNoFiles, PlanDeferralLoopTemplate,
        PlanOverallSuccessWithDeferralsTemplate, PullStateCleanTemplate, PullStateErrorTemplate,
        PullStateSuccessWithDeferralsTemplate,
    },
};
use crate::{
    changeset::{util::create_comment_standalone, ChangeSet},
    command::{AutoschematicCommand, HELP},
    error::{AutoschematicServerError, AutoschematicServerErrorType},
    template::{
        self, random_failure_emoji, ApplyErrorTemplate, ApplyNoPlanTemplate, ApplySuccessTemplate,
        CommandParseFailure, GreetingTemplate, ImportErrorTemplate, ImportSuccessTemplate,
        PlanErrorTemplate, PlanNoChangesTemplate, PlanOverallErrorTemplate,
        PlanOverallSuccessTemplate, PlanSuccessTemplate, PrLockHeld, SkeletonImportErrorTemplate,
        SkeletonImportSuccessTemplate,
    },
    DOMAIN,
};
use crate::{template::PullStateSuccessTemplate, TASK_REGISTRY};
...
pub async fn dispatch(webhook_event: WebhookEvent) -> Result<(), AutoschematicServerError> {
    ...
    if let Ok(Some(task_message)) = from_github_webhook(&webhook_event) {
                ...
                Ok(cmd) => {
                    ...
                    let mut changeset = match changeset.try_lock() {
                        ...
                        Err(e) => {
                            ...
                            create_comment_standalone(&webhook_event, &error_template.render()?)
                                .await?;
                            return Err(e.into());
                        }
                    };
                    ...
                    let _ = changeset
                        .add_reaction(
                            comment_id,
                            octocrab::models::reactions::ReactionContent::Rocket,
                        )
                        .await;
                            ...
                            match res {
                                ...
                                Err(e) => {
                                    ...
                                    let import_error_template = ImportErrorTemplate {
                                        error_message: try_unescape(&format!("{:#}", e))
                                            .to_string(),
                                        failure_emoji: template::random_failure_emoji(),
                                    };
                                            ...
                                            if let Some(error) = &plan_report.error {
                                                ...
                                                for op in &plan_report.connector_ops {
                                                    if let Some(msg) = &op.friendly_message {
                                                        op_reports.push((
                                                            msg.to_string(),
                                                            op.op_definition.to_string(),
                                                        ));
                                                    } else {
                                                        op_reports.push((
                                                            String::from("Op Summary"),
                                                            op.op_definition.to_string(),
                                                        ));
                                                    }
                                                }
                                            ...
                                            if all_plans_empty {
                                                if plan_report_set.deferred_count > 0 {
                                                    let msg = PlanDeferralLoopTemplate {
                                                        failure_emoji:
                                                            template::random_failure_emoji(),
                                                        deferred_count: plan_report_set
                                                            .deferred_count,
                                                ...
                                                let apply_command = match (&subpath, &connector) {
                                                    (None, None) => {
                                                        String::from("autoschematic apply")
                                                    }
                                                    (None, Some(connector)) => {
                                                        format!(
                                                            "autoschematic apply -c {}",
                                                            &connector
                                                        )
                                                    }
                                                    (Some(subpath), None) => {
                                                        format!(
                                                            "autoschematic apply -p {:?}",
                                                            subpath
                                                        )
                                                    }
                                                ...
                                                if plan_report_set.deferred_count > 0 {
                                                    let msg =
                                                        PlanOverallSuccessWithDeferralsTemplate {
                                                            success_emoji:
                                                                template::random_success_emoji(),
                                                            apply_command,
                                                            deferred_count: plan_report_set
                                    ...
                                    for apply_report in apply_report_set.apply_reports {
                                        if apply_report.error.is_none() {
                                            ...
                                            let apply_success_template = ApplySuccessTemplate {
                                                success_emoji: template::random_success_emoji(),
                                                filename: String::from(
                                                    apply_report.virt_addr.to_string_lossy(),
                                                ),
                                            ...
                                            let apply_error_template = if let Some(error) =
                                                apply_report.error
                                            {
                                                overall_success = false;
                                                ApplyErrorTemplate {
