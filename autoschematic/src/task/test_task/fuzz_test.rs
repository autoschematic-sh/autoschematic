...
use crate::{
    ...
    git_util::{
        checkout_branch, checkout_new_branch, get_head_sha, git_add, git_commit_and_push,
        pull_with_rebase,
    },
...
impl TestTask {
    pub async fn run_fuzz_test(&mut self, path: &Path) -> anyhow::Result<()> {
        ...
        let fuzz_config: FuzzConfig =
            RON.from_str(&std::fs::read_to_string(path.join("fuzz_config.ron"))?)?;

        // let mut rng = rand::thread_rng();
        // fuzz_config.states.shuffle(&mut rng);
        let fuzz_config = fuzz_config.clone();

        let connector_filter = fuzz_config
            .connector_filter
            .map(|c| format!("-c {}", c))
            .unwrap_or_default();
        ...
        loop {
            ...
            for state in &fuzz_config.states {
                ...
                git_commit_and_push(
                    ...
                    &branch_name,
                    &self.token,
                    &format!("Fuzz state {}", state),
                )?;

                self.plan_apply_pull_state_complete(
                    issue_number,
                    &prefix_filter,
                    &connector_filter,
                    &repo_path,
