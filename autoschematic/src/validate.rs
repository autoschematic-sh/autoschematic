...
pub fn validate() -> anyhow::Result<()> {
    ...
    let _config: AutoschematicConfig = RON
        .from_str(&std::fs::read_to_string(&config_path).context("Reading autochematic.ron")?)
    ...
    if rbac_config_path.is_file() {
        let _rbac_config: AutoschematicRbacConfig = RON
            .from_str(&std::fs::read_to_string(&config_path).context("Reading autochematic.rbac.ron")?)
